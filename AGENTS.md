...
## Project-specific Phase 0 rules

- Project: `ecommerce-ops-refund-dashboard`.
- Root: `C:\Users\alex\Documents\Coding Projects\Portfolio Projects\ecommerce-ops-refund-dashboard`.
- Stack: Next.js App Router, TypeScript, Tailwind CSS v4, shadcn/ui Radix Nova, Prisma 7, local PostgreSQL through Docker Compose, Vitest, Playwright.
- Package manager: pnpm only.
- Phase 0 scope: scaffold, tooling, local database guardrails, docs, and smoke validation only.
- Phase 0 out of scope: KPI cards, order tables, refunds/disputes pages, customer pages, CSV import/export, Stripe handlers, alert rules, seed/demo business data, and GitHub Actions.
- Integration default: mock/no-paid-API mode. Stripe test webhooks require explicit user approval before use.
- Data policy: never use real Stripe, Shopify, WooCommerce, customer, order, payment, or refund data.
- Secrets policy: keep `.env.local` untracked; use `.env.example` with placeholders only.
- Git policy: Codex must never run `git add`, `git commit`, `git push`, create tags, rewrite history, delete branches, or alter remotes unless the user explicitly approves that exact action.
- Validation commands: `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, `pnpm e2e`, and `git status --short`.
...
- Smoke/manual check: `pnpm dev`, then open `http://localhost:3000`
...
- OS: Windows 11 Pro.
- Project root: `C:\Users\alex\Documents\Coding Projects\Portfolio Projects\ecommerce-ops-refund-dashboard`.
